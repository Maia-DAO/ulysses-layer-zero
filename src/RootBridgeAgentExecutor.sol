...
import {IRootBridgeAgent, GasParams} from "./interfaces/IRootBridgeAgent.sol";
...
contract RootBridgeAgentExecutor is Ownable {
     ...
     */
    function executeSystemRequest(address _router, bytes calldata _data, uint16 _fromChainId)
        external
        payable
        onlyOwner
    {
        //Try to execute remote request
        IRouter(_router).executeResponse{value: msg.value}(bytes1(_data[PARAMS_TKN_START]), _data[6:], _fromChainId);
    }
     ...
     */
    function executeNoDeposit(address _router, bytes calldata _data, uint16 _fromChainId) external payable onlyOwner {
        //Execute remote request
        IRouter(_router).execute{value: msg.value}(bytes1(_data[PARAMS_TKN_START]), _data[6:], _fromChainId);
    }
     ...
     */
    function executeWithDeposit(address _router, bytes calldata _data, uint16 _fromChainId)
        external
        payable
        onlyOwner
    {
        ...
        if (_data.length > 109) {
            //Execute remote request
            IRouter(_router).executeDepositSingle{value: msg.value}(_data[109], _data[110:], dParams, _fromChainId);
        }
    }
     ...
     */
    function executeWithDepositMultiple(address _router, bytes calldata _data, uint16 _fromChainId)
        external
        payable
        onlyOwner
    {
        ...
        if (length > PARAMS_END_OFFSET + uint16(uint8(bytes1(_data[PARAMS_START]))) * PARAMS_TKN_SET_SIZE_MULTIPLE) {
            //Try to execute remote request
            IRouter(_router).executeDepositMultiple{value: msg.value}(
                ...
                _data[PARAMS_START + PARAMS_END_OFFSET + uint16(numOfAssets) * PARAMS_TKN_SET_SIZE_MULTIPLE:],
                dParams,
                _fromChainId
            );
        }
     ...
     */
    function executeSignedNoDeposit(address _account, address _router, bytes calldata _data, uint16 _fromChainId)
        external
        payable
        onlyOwner
    {
        //Execute remote request
        IRouter(_router).executeSigned{value: msg.value}(_data[25], _data[26:], _account, _fromChainId);
    }
     ...
     */
    function executeSignedWithDeposit(address _account, address _router, bytes calldata _data, uint16 _fromChainId)
        external
        payable
        onlyOwner
    {
        ...
        if (_data.length > 129) {
            ...
            IRouter(_router).executeSignedDepositSingle{value: msg.value}(
                _data[129], _data[130:], dParams, _account, _fromChainId
            );
        }
    }
     ...
     * @param _gParams Gas parameters for the retry request.
     * @dev DEPOSIT FLAG: 7 (Retry Settlement)
     */
    function executeRetrySettlement(uint32 _settlementNonce, GasParams calldata _gParams) external payable onlyOwner {
        //Execute remote request
        RootBridgeAgent(payable(msg.sender)).retrySettlement{value: msg.value}(_settlementNonce, _gParams);
    }
     ...
     *
     */
    function _bridgeIn(address _recipient, DepositParams memory _dParams, uint16 _fromChain) internal {
     ...
     *
     */
    function _bridgeInMultiple(address _recipient, bytes calldata _dParams, uint16 _fromChain)
        internal
