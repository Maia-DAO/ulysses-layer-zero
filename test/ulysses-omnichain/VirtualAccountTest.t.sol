...
contract VirtualAccountTest is DSTestPlus {
        ...
        returns (VirtualAccount virtualAccount)
    {
        ...
        (, bytes memory userReturn) = address(virtualAccount).call(
            LibZip.cdCompress(abi.encodeWithSelector(IVirtualAccount.userAddress.selector))
        );
        address userAddress = abi.decode(userReturn, (address));

        (, bytes memory localPortReturn) = address(virtualAccount).call(
            LibZip.cdCompress(abi.encodeWithSelector(IVirtualAccount.localPortAddress.selector))
        );
        address localPort = abi.decode(localPortReturn, (address));

        assertEq(userAddress, _userAddress);
        assertEq(localPort, _localPortAddress);
    }
    ...
    function test_requiresApprovedCaller_withdrawNative_LibZip(address _userAddress, uint256 _withdrawAmount)
        public
        returns (VirtualAccount virtualAccount)
    {
        if (_userAddress == address(this)) _userAddress = address(1);
    ...
    function test_requiresApprovedCaller_withdrawERC20_LibZip(
    ...
    function test_requiresApprovedCaller_withdrawERC721_LibZip(address _userAddress, address _token, uint256 _tokenId)
        public
        returns (VirtualAccount virtualAccount)
    {
        if (_userAddress == address(this)) _userAddress = address(1);
    ...
    function test_requiresApprovedCaller_call_LibZip(address _userAddress, Call[] calldata _calls)
        public
        returns (VirtualAccount virtualAccount)
    {
        if (_userAddress == address(this)) _userAddress = address(1);
    ...
    function test_requiresApprovedCaller_payableCall_LibZip(address _userAddress, PayableCall[] calldata _calls)
        public
        returns (VirtualAccount virtualAccount)
    {
        if (_userAddress == address(this)) _userAddress = address(1);
    ...
    function test_withdrawNative_LibZip(address _userAddress, uint256 _depositAmount, uint256 _withdrawAmount)
        public
        returns (VirtualAccount virtualAccount)
    {
        (_depositAmount, _withdrawAmount) = _parseDepositAndWithdrawAmounts(_depositAmount, _withdrawAmount);
    ...
    function test_withdrawNative_Unautharized_LibZip(
        address _userAddress,
    ...
    function test_withdrawERC20_LibZip(
        address _userAddress,
        bytes32 _tokenSalt,
        uint256 _depositAmount,
        uint256 _withdrawAmount
    ...
    function test_withdrawERC20_Unautharized_LibZip(
        address _userAddress,
        bytes32 _tokenSalt,
        uint256 _depositAmount,
        uint256 _withdrawAmount
    ...
    function test_withdrawERC721_LibZip(address _userAddress, bytes32 _tokenSalt, uint256 _tokenId) public {
        (VirtualAccount virtualAccount, MockERC721 mockERC721) =
            _testReceiveERC721(_userAddress, _tokenSalt, localPortAddress, _tokenId);
    ...
    function test_OnERC721Received_LibZip() public {
        VirtualAccount virtualAccount = _deployVirtualAccount(address(this), localPortAddress);

        (, bytes memory returnData) = address(virtualAccount).call(
            LibZip.cdCompress(
                abi.encodeWithSelector(IERC721Receiver.onERC721Received.selector, address(0), address(0), 0, "")
            )
        );
    ...
    function test_withdrawERC1155_LibZip(address _userAddress, bytes32 _tokenSalt, uint256 _tokenId, uint256 _amount)
        public
    {
        (VirtualAccount virtualAccount, MockERC1155 mockERC1155) =
            _testReceiveERC1155(_userAddress, _tokenSalt, localPortAddress, _tokenId, _amount);
    ...
    function test_withdrawERC1155_Unauthorized_LibZip(
        address _userAddress,
        bytes32 _tokenSalt,
        uint256 _tokenId,
        uint256 _amount
    ...
    function test_OnERC1155Received_LibZip() public {
        VirtualAccount virtualAccount = _deployVirtualAccount(address(this), localPortAddress);

        (, bytes memory returnData) = address(virtualAccount).call(
            LibZip.cdCompress(
                abi.encodeWithSelector(IERC1155Receiver.onERC1155Received.selector, address(0), address(0), 0, 0, "")
            )
        );
    ...
    function test_OnERC1155BatchReceived_LibZip() public {
        VirtualAccount virtualAccount = _deployVirtualAccount(address(this), localPortAddress);
    ...
    function test_call_LibZip() public {
        ...
        (, bytes memory ret) = address(virtualAccount).call(
            LibZip.cdCompress(abi.encodeWithSelector(IVirtualAccount.call.selector, calls))
        );
    ...
    function test_call_unsuccessful_LibZip() public {
        address userAddress = address(this);
        VirtualAccount virtualAccount = _deployVirtualAccount(userAddress, localPortAddress);
    ...
    function test_call_isEOA_LibZip() public {
        address userAddress = address(this);
    ...
    function test_payableCall_singleCall_LibZip() public {
        address userAddress = address(this);
    ...
    function test_payableCall_isEOA_LibZip() public {
        address userAddress = address(this);
    ...
    function test_payableCall_notAllSpent_LibZip() public {
        address userAddress = address(this);
    ...
    function test_payableCall_LibZip() public {
        address userAddress = address(this);
        VirtualAccount virtualAccount = _deployVirtualAccount(userAddress, localPortAddress);
        ...
        (, bytes memory ret) = address(virtualAccount).call{value: 1}(
            LibZip.cdCompress(abi.encodeWithSelector(IVirtualAccount.payableCall.selector, calls))
        );
    ...
    function test_payableCall_unsuccessful_LibZip() public {
        address userAddress = address(this);
        VirtualAccount virtualAccount = _deployVirtualAccount(userAddress, localPortAddress);
    ...
    function _withdrawERC1155UsingCall_LibZip(
        VirtualAccount virtualAccount,
        address erc1155Token,
        uint256 tokenId,
        uint256 amount,
